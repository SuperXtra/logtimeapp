...
class OverallStatisticsReportQueryTest extends QueryTest {
  ...
  test("Statistics report query test"){
    check(StatisticsReportQuery(MainReport(Some(List("qwerty", "trewq")), Some(DateFilter(Year(2020), Month(1))), Some(DateFilter(Year(2025), Month(1))))))
  }
}
